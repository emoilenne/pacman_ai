...
class PacmanGraphics:
    ...
    def moveGhost(self, ghost, ghostIndex, prevGhost, ghostImageParts, prevPos=None, newPos=None):
        ...
        old_x, old_y = self.to_screen(prevPos)
        new_x, new_y = self.to_screen(newPos)
    ...
    def animateGhost(self, ghost, ghostIndex, prevGhost, ghostImageParts):
        if self.frameTime > 0.01 or self.frameTime < 0:
            ...
            px, py = self.getPosition(ghost)
            frames = 10.0
            prevPos = (fx, fy)
            for i in range(1,int(frames) + 1):
                pos = px*i/frames + fx*(frames-i)/frames, py*i/frames + fy*(frames-i)/frames
                self.moveGhost(ghost, ghostIndex, prevGhost, ghostImageParts, prevPos=prevPos, newPos=pos)
                prevPos = pos
                refresh()
                # sleep(abs(self.frameTime) / frames)
        else:
            self.moveGhost(self.getPosition(ghost), self.getDirection(ghost), ghostImageParts)
        refresh()
